Millions of developers install Fastify packages every week. That trust is not something we take lightly, so we spent the last months hardening how the organization publishes code.
...
* **Dependency scripts**: two lines in `.npmrc` do a surprising amount of work. `ignore-scripts=true` stops lifecycle scripts from running on install, and `min-release-age=2` refuses versions younger than two days, which is usually enough time for a malicious release to be spotted and unpublished.
...
#Fastify #NodeJS #OpenSource #Security #SupplyChainSecurity #DevSecOps
